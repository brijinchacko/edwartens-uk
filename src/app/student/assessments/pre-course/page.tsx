...
export default function PreCourseQuizPage() {
  ...
  const startQuiz = useCallback(async () => {
    try {
      const res = await fetch("/api/student/assessments/start", {
        ...
        body: JSON.stringify({ type: "PRE_COURSE" }),
      });
  ...
  // Results screen
  if (result) {
    const areas = result.suggestions?.areas || [];
    return (
      <div className="space-y-6">
        ...
        <div className="glass-card p-6 text-center space-y-4">
          <CheckCircle2 size={48} className="mx-auto text-neon-blue" />
          <h2 className="text-lg font-bold text-text-primary">
            Pre-Course Assessment Complete
          </h2>
          ...
          <p className="text-sm text-text-muted max-w-md mx-auto">
            This is a diagnostic assessment. Your results help us tailor the
